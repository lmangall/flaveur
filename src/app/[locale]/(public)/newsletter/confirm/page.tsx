...
export default async function ConfirmNewsletterPage({ searchParams, params }: Props) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-2xl font-bold mb-4">{t("confirmationSuccess")}</h1>
          <p className="text-muted-foreground mb-6">{t("confirmationSuccessDescription")}</p>
          <Link href={`/${locale}`}>
            <Button>{t("backToHome")}</Button>
          </Link>
        </div>
  ...
  let title = t("confirmationError");
  let description = t("confirmationErrorDescription");

  if (result.error === "already_confirmed") {
    title = t("alreadyConfirmed");
