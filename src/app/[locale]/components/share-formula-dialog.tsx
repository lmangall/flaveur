...
export function ShareFormulaDialog({ formulaId, formulaName }: ShareFormulaDialogProps) {
  ...
  const loadShares = useCallback(async () => {
    ...
    try {
      const data = await getFormulaShares(formulaId);
      setShares(data);
    } catch (error) {
  ...
  const handleShare = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await shareFormula({ formulaId, email: email.trim(), locale });
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("shareDescription", { name: formulaName })}
          </DialogDescription>
        </DialogHeader>
