...
import {
  ...
  createFormulaWithSubstancesById,
...
export default function ImportFormulationPage() {
  ...
  const [createdFormulaName, setCreatedFormulaName] = useState<string | null>(null);
  ...
  const handleExtractionComplete = useCallback((result: VisionExtractionResult) => {
    // Update metadata
    if (result.metadata.studentName) setStudentName(result.metadata.studentName);
    if (result.metadata.formulaName) setFormulaName(result.metadata.formulaName);
    if (result.metadata.formulationDate) setFormulationDate(result.metadata.formulationDate);

    // Update rows with extracted ingredients
    const newRows: IngredientRow[] = result.ingredients.map((ing, idx) => ({
      id: idx + 1,
      name: ing.name,
      supplier: ing.supplier || "",
      dilution: ing.dilution || "",
      pricePerKg: ing.pricePerKg?.toString() || "",
      concentrationA: ing.concentrationA?.toString() || "",
      concentrationB: ing.concentrationB?.toString() || "",
      concentrationC: ing.concentrationC?.toString() || "",
      concentrationD: ing.concentrationD?.toString() || "",
    }));

    // Pad to minimum 18 rows
    while (newRows.length < 18) {
      newRows.push({ id: newRows.length + 1, ...EMPTY_ROW });
    }
    ...
    if (result.warnings.length > 0) {
      toast.warning(`${result.warnings.length} warnings during extraction`, {
        description: result.warnings.slice(0, 3).join(", "),
      });
    }

    toast.success("Formulation data imported from photo");
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const formulaResult = await createFormulaWithSubstancesById({
      ...
      setCreatedFormulaId(formulaResult.formula_id);
      setCreatedFormulaName(formulaResult.name);
      setStep("result");
      fireConfetti();
      toast.success(`Formula "${formulaResult.name}" created successfully!`);
    } catch (error) {
          ...
          <Card className="p-6 border-green-500">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Formula "{createdFormulaName}" has been created with {confirmedIngredients.length} substances.
                </p>
              </div>
            </div>
          </Card>
          ...
          <div className="flex justify-between">
            <Button
              ...
              onClick={() => {
                ...
                setFormulationDate("");
                setConfirmedIngredients([]);
                setCreatedFormulaId(null);
                setCreatedFormulaName(null);
              }}
            >
              Import Another
            </Button>
            {createdFormulaId && (
              <Button onClick={() => router.push(`/formulas/${createdFormulaId}`)}>
                View Formula
                <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            )}
