...
export default function FlavorClient({ flavor }: { flavor: Formula }) {
  const { data: session } = useSession();
  const { mutate } = useSWR(`/api/formulas/${flavor.formula_id}`);
  ...
  const handleRemoveSubstance = async (substanceId: number) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/formulas/${flavor.formula_id}/substances/${substanceId}`,
        {
          method: "DELETE",
